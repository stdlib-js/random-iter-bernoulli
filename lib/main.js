...
var rbern = require( '@stdlib/random-base-bernoulli' ).factory;
...
function iterator( p, options ) {
	...
	var i;
	if ( !isProbability( p ) ) {
		throw new TypeError( format( '0pH6y,NS', p ) );
	}
	if ( arguments.length > 1 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pH2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pH2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
